...
if (isset($_POST["delete"])) {
  if (isset($_POST["email"])) {
    ...
    if (isset($_POST["isDepartment"]) && $_POST['isDepartment'] == true) {
      ...
      if ($delete_result) {
        ...
        $error_message = mysqli_error($connection);
        echo "Diagnosis not deleted because doctor has select this diagnosis";
      }
    } else {
  ...
  // Get form data
  $patientId = isset($_SESSION['id']) ? $_SESSION['id'] : '';
  $role =  isset($_SESSION['id']) ? $_SESSION['id'] : "patients";
  $patientName = isset($_POST["first_name"]) ? $_POST["first_name"] : null;
  ...
  if ($email === null && $role === "patients") {
    // Book appointment directly when email is not received
    ...
    if (mysqli_num_rows($result) > 0) {
      ...
      if (mysqli_query($connection, $userSql)) {
        ...
        VALUES ('$first_name','$last_name', '$email', 1, '$password')";

        if (mysqli_query($connection, $patientSql)) {
          $patientId = mysqli_insert_id($connection);
          // Insert data into the appointments table
          $appointmentSql = "INSERT INTO appointments (patient_id, patient_name, diagnosis,status, doctor_id, opd_date, description)
                              VALUES ($patientId, '$patientName', '$diagnosis','Pending', '$doctorId', '$opdDate', '$description')";
  ...
  header("location: /hospital/doctor/view-appointment.php?id=$appointment_id");
  exit;
}
